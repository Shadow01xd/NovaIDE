...
export class AIAgent {
  ...
  attachEventListeners() {
    ...
    input.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { e.preventDefault(); this.handleSend(); }
      if (e.key === '@' && !e.ctrlKey && !e.metaKey) this.container.querySelector('#ai-btn-attach').click();
    });
    ...
    // Provider change
    this.container.querySelector('#ai-provider-select').addEventListener('change', (e) => {
    ...
    // Model change
    this.container.querySelector('#ai-model-select').addEventListener('change', (e) => {
    ...
    // API keys
    this.container.querySelector('#ai-apikey-input')?.addEventListener('change', (e) => {
      this.deepseekApiKey = e.target.value;
      localStorage.setItem('ide_deepseek_key', this.deepseekApiKey);
    });
    this.container.querySelector('#ai-groqkey-input')?.addEventListener('change', (e) => {
      this.groqApiKey = e.target.value;
      localStorage.setItem('ide_groq_key', this.groqApiKey);
    });

    // Confirm delete toggle
    this.container.querySelector('#ai-confirm-delete')?.addEventListener('change', (e) => {
      this.confirmDelete = e.target.checked;
    });
    this.container.querySelector('#ai-confirm-commands')?.addEventListener('change', (e) => {
    ...
    this.container.querySelectorAll('.ai-suggestion-btn').forEach(btn =>
      btn.addEventListener('click', () => { input.value = btn.dataset.prompt; this.handleSend(); })
    );
  ...
  async loadModelList() {
    ...
    if (this.provider === 'ollama') {
      try {
        ...
        if (!models.length) {
          select.innerHTML = '<option value="deepseek-coder">deepseek-coder (sin modelos)</option>';
        } else {
          models.forEach(m => {
            const opt = document.createElement('option');
            opt.value = opt.textContent = m;
            if (m === this.activeModel || (!this.activeModel && m.includes('deepseek-coder'))) opt.selected = true;
        ...
        select.innerHTML = '<option value="deepseek-coder">deepseek-coder (Ollama offline)</option>';
      }
      ...
      `;
      select.value = this.activeModel || 'meta-llama/llama-4-scout-17b-16e-instruct';
    } else {
      select.innerHTML = `
...
replace_selection   {"tool":"replace_selection","params":{"newText":"code"}}`;
  ...
  async streamResponse(iteration) {
    ...
    try {
      ...
      const toolCalls = this.activeTab === 'agent' ? this.parseToolCalls(fullResponse) : [];
      ...
      if (err.name === 'AbortError') {
        ...
        if (c) {
          ...
          c.innerHTML = v ? renderMarkdown(v) : '<em style="opacity:.5">Generación detenida</em>';
        }
        return;
      }
      const c = msgEl.querySelector('.ai-msg-content');
  ...
  async streamDeepSeek(messages, onToken, signal) {
    if (!this.deepseekApiKey) throw new Error('API Key de DeepSeek no configurada. Abre ⚙ Configuración para añadirla.');
    await this._streamViaIPC('deepseek', messages, onToken, signal);
  }

  async streamGroq(messages, onToken, signal) {
    if (!this.groqApiKey) throw new Error('API Key de Groq no configurada. Abre ⚙ Configuración para añadirla.');
  ...
  shouldConfirmToolCall(tool, params = {}) {
    ...
    if (tool === 'move_file') {
      return !this.isPathInWorkspace(params.source) || !this.isPathInWorkspace(params.destination);
    }
    return false;
  }
  ...
  async executeTool(tool, params) {
    ...
    switch (tool) {
      case 'read_file': {
        ...
        this.ensureWorkspacePath(fp, 'No se puede leer fuera de la carpeta abierta.');
      ...
      case 'delete_file': {
        const fp = resolve(params.path);
        this.ensureWorkspacePath(fp, 'No se puede eliminar fuera de la carpeta abierta.');
      ...
      case 'move_file': {
        ...
        this.ensureWorkspacePath(source, 'No se puede mover archivos fuera de la carpeta abierta.');
        this.ensureWorkspacePath(destination, 'No se puede mover archivos fuera de la carpeta abierta.');
      ...
      case 'search_in_files': {
        const dir = params.directory ? resolve(params.directory) : this.state.currentFolder;
        if (!dir) throw new Error('No hay carpeta abierta.');
        this.ensureWorkspacePath(dir, 'No se puede buscar fuera de la carpeta abierta.');
      ...
      case 'open_file': {
        const fp = resolve(params.path);
        this.ensureWorkspacePath(fp, 'No se puede abrir fuera de la carpeta abierta.');
        const r = await window.api.agentReadFile(fp);
        if (!r.success) throw new Error(r.error);
  ...
  updateStreamingMessage(el, text) {
    ...
    if (!c) return;
    c.innerHTML = (text ? renderMarkdown(text) : '') + '<span class="ai-cursor">▋</span>';
    this.scrollToBottom();
  }
