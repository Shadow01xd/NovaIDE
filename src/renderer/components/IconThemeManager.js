...
export class IconThemeManager {
    ...
    async loadIconTheme() {
        try {
            ...
            if (!contributions || !contributions.iconThemes || contributions.iconThemes.length === 0) {
    ...
    _applyIconTheme(info, data) {
        ...
        const themeDir = info.path.substring(0, info.path.lastIndexOf(isWindows ? '\\' : '/'));
        ...
        for (const [name, iconName] of Object.entries(folderNames)) {
            ...
            if (expIconName && defs[expIconName]) {
                 css += this._generateIconCSS(`fdir-${name}-open`, defs[expIconName], info.path);
            }
        }
        ...
        if (data.folder && defs[data.folder]) {
            ...
            if (data.folderExpanded && defs[data.folderExpanded]) {
                css += this._generateIconCSS('d-default-open', defs[data.folderExpanded], info.path);
            }
        }
    ...
    _generateIconCSS(selector, def, themePath) {
        ...
        if (def.fontCharacter) {
            const content = def.fontCharacter.startsWith('\\') ? def.fontCharacter : `\\${def.fontCharacter}`;
            style = `
    ...
    _resolvePath(basePath, relativePath) {
        ...
        console.log(`[IconThemeManager] Resolved: ${basePath} + ${relativePath} -> ${resolved}`);
        return resolved;
    }
    ...
    getIconClass(name, isDir, isOpen) {
        ...
        if (isDir) {
            ...
            let base = this.themeData.folderNames?.[nameLower] ? `fdir-${nameLower}` : 'd-default';
            if (isOpen) {
                // Check if there is an open version
                if (this.themeData.folderNamesExpanded?.[nameLower]) return `fdir-${nameLower}-open`;
        ...
        if (this.themeData.fileNames?.[name.toLowerCase()]) return `fname-${safeName}`;
