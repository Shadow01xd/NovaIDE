...
class MarketplaceService {
  ...
  async extensionQuery(payload) {
    ...
    if (!response.ok) throw new Error(`Marketplace API error: ${response.status} ${response.statusText}`);
  ...
  async searchExtensions(query) {
    ...
    if (query) {
      if (query.startsWith('category:')) {
        criteria.push({ filterType: 5, value: query.replace('category:', '').trim() });
      } else if (query.startsWith('tag:')) {
        criteria.push({ filterType: 1, value: query.replace('tag:', '').trim() });
      } else {
  ...
  async downloadExtension(publisher, name, version) {
    ...
    if (!response.ok) throw new Error(`Failed to download: ${response.statusText} (${url})`);
  ...
  async installExtension(publisher, name, version) {
    ...
    const meta = { id, publisher, name, version, installedAt: new Date().toISOString() };
